...
st.title("Connect Four AI - Amateur Level")
...
# This function is to tell whether or not a space is full and a piece could be placed.
def is_valid_location(board, col):
    return board[0][col] == 0

# This return all the open columns
def get_valid_locations(board):
    return [col for col in range(COLS) if is_valid_location(board, col)]

# This returns the next open row for a given column
...
# This gives the score of a board depending on center position
def score_position(board, piece):
    center = [int(i) for i in list(board[:, COLS//2])]
    center_count = center.count(piece)
    return center_count * 3

# This is our minimax algorithm with alpha-beta pruning
...
# This is the AI using minimax and alpha-beta pruning (depth 3)
if st.session_state.turn == 2:
    ...
    if valid_cols:
        ...
        if check_winner(st.session_state.board, 2):
            st.success("🟡 AI Wins!")
            st.session_state.turn = 0  # Checks if AI wins the game 
        else:
            st.session_state.turn = 1  
